...
class Project(models.Model):
    ...
    type = models.CharField(
        max_length=20, choices=project_type, default=0, verbose_name=u'项目类型')
    ...
    remark = models.TextField(
        max_length=200, blank=True, null=True, verbose_name=u'备注')
    ...
    def get_all_project(self):
        project = Project.objects.all().values_list('id','name')
        return project
...
class Module(models.Model):
    ...
    belong_project = models.ForeignKey(
    ...
    remark = models.TextField(
        max_length=200, verbose_name=u'备注', blank=True, null=True)
    add_time = models.DateField(auto_now_add=True, verbose_name=u'创建时间')
    edit_time = models.DateField(auto_now=True, verbose_name=u'修改时间')
...
class Suites(models.Model):
    ...
    belong_module = models.ForeignKey(
        Module, verbose_name=u'所属模块', on_delete=models.SET_NULL, null=True, blank=True)
    creator = models.ForeignKey(
        UserProfile, verbose_name=u'创建人', on_delete=models.SET_NULL, null=True, blank=True)
...
class Steps(models.Model):
    ...
    doc = models.CharField(
        max_length=50, verbose_name=u'操作步骤描述', null=True, blank=True)
    remark = models.TextField(max_length=200, verbose_name=u'备注', blank=True)
    add_time = models.DateField(auto_now_add=True, verbose_name=u'创建时间')
    edit_time = models.DateField(auto_now=True, verbose_name=u'修改时间')
...
class CasesStep(models.Model):
    ...
    case_id = models.ForeignKey(
        Cases, verbose_name=u'测试用例', on_delete=models.SET_NULL, null=True, blank=True)
    step_seqnum = models.IntegerField(default=1, verbose_name=u'操作步骤顺序号')
    step_id = models.ForeignKey(
        Steps, verbose_name=u'操作步骤', on_delete=models.SET_NULL, null=True, blank=True)
    add_time = models.DateField(auto_now_add=True, verbose_name=u'创建时间')
    edit_time = models.DateField(auto_now=True, verbose_name=u'修改时间')
